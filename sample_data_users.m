...
% features corresponding to average values over 5 seconds, thus we expect
% [num_users * 12 * 10 * 60 / 5] = [2080 * num_users] discrete
% observations.
% To prevent memory fragmentation, we will preallocate twice that amount then
% trim the resulting rows of zeros.
expected_rows = 4160 * num_users;
%y_results: a vector of 6-digit IDs
y_results = zeros(expected_rows, 1);

% X_input: the N x d design matrix
X_input = zeros(expected_rows, num_features);

session = zeros(expected_rows, 1);
...
for i = 1:num_users
    ...
    for j = 1:num_sessions
        ...
        %That number is in the first row, 2th column (0,1)
        subjectID = csvread(activity_file, 0, 1, [0 1 0 1]);
        ...
        %statistics
        windows = getIntervals(accel_time, 5); %five second windows
        ...
        y_results(row_num:row_num + n - 1) = repmat(subjectID, [n 1]);
        
        catS = categorical(cellstr(s))
        session(row_num:row_num + n - 1) = repmat(catS, [n 1]);
...
%remove any errant NaNs 
[rows, columns] = find(isnan(X_input));
rows = unique(rows);
X_input(rows, :) = [];


%save('user_ident_preprocessed_v3', 'X_input', 'y_results', 'user', 'session')
