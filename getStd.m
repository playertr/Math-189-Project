function [ newA] = getStd(input_matrix, time, windows)
%function getStd: this takes in an mxn matrix (where m is ~70000
...
% standard deviation of the input matrxi over the nonoverlapping windows.
...
for i = 1:newA_length % for every row in the new matrix
    ...
    %this row becomes the std dev of those rows of accel_matrix
    newA(i, :) = std( input_matrix(startIndex :endIndex, :) );
